...
class PPO:
    ...
    def __init__(
        ...
        """
        PPO implementation based on "Proximal Policy Optimization \
    ...
    def update(self, minibatch_sampler: MetaBatchSampler) -> PPOUpdate:
        ...
        Args:
          minibatch_sampler (RolloutStorage): Rollouts to be used as data 
          points for making updates.
        ...
        #     minibatches = minibatch_sampler.sample(advantages, \
        #                                            self.num_minibatches)
        # Wrapping the outer loop with tqdm for optimization epochs
        for e in tqdm(range(self.opt_epochs), desc="Optimization Epochs", leave=False):
            minibatches = minibatch_sampler.sample(advantages, self.num_minibatches)

            # # print(minibatches)
            # for sample in minibatches:
            # for sample in tqdm(minibatches, leave=False, desc="Processing minibatches"):
            for sample in minibatches:
                (
                    obs_batch,
                    actor_states_batch,
