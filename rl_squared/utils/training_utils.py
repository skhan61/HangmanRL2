...
def sample_meta_episodes(
    ...
    Args:
        ...
        rl_squared_envs (PyTorchVecEnvWrapper): Parallel environments for sampling 
        episodes.
        meta_episode_length (int): Meta-episode length, each "meta-episode" 
    ...
    # for _ in range(num_meta_episodes // num_parallel_envs): # num_meta_episodes == # word/task
    # Wrapping the loop with tqdm and adding a description
    for _ in tqdm(range(num_meta_episodes // num_parallel_envs), desc="Processing Corpus", leave=False):
        meta_episodes = MetaEpisodeBatch(
            meta_episode_length,
            num_parallel_envs,  # taking number of num_parallel_envs metaepisodes at a time
            observation_space,
            action_space,
        ...
        # print(meta_episodes.recurrent_states_actor[0].shape)

        # print(initial_observations.shape)
        meta_episodes.obs[0].copy_(initial_observations)

        for step in range(meta_episode_length):  # per task how many steps
        # # # Wrapping the step loop with tqdm for better visibility of progress
        # # for step in tqdm(range(meta_episode_length), desc="Processing Steps", leave=False):        
            action_masks = rl_squared_envs.action_masks() # action masks
            # print(action_masks)
            (
                value_preds,
                actions,
        ...
        meta_episodes.compute_returns(
            next_value_pred, use_gae, discount_gamma, \
                gae_lambda
        )
    ...
    mean_reward_per_step = np.sum(episode_rewards) \
        / total_env_steps
