...
class BernoulliBanditEnv(EzPickle, BaseMetaEnv):
    def __init__(
        ...
        Args:
            ...
            auto_reset (bool): Whether to `auto_reset` 
                    and the end of episode.
    ...
    def reset(
        ...
        """
        Resets the environment and returns the \
          corresponding observation.

        This is different from `sample_task`, unlike the former 
        this will not change the payout probabilities.
    ...
    def step(self, action: int) -> Tuple:
      """
      Take a step in the environment and return the 
      corresponding observation, action, reward,
      additional info, etc.
      ...
      reward = self.np_random.binomial(n=1, \
                            p=self._payout_odds[action], size=1)[0]
