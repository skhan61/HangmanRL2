...
#         max_len = max(len(x[0]) for x in batch)
#         padded_value = max(max(word[0]) for word in batch) + 1
#         inputs = torch.stack([torch.tensor(np.pad(word[0], (0, max_len \
#                         - len(word[0])), mode='constant', constant_values=padded_value))\
#                               for word in batch])
#         labels = torch.stack([torch.tensor(x[1]) for x in batch])
#         miss_chars = torch.stack([torch.tensor(x[2]) for x in batch])
#         lens = torch.tensor([len(x[0]) for x in batch], dtype=torch.long)
...
#         inputs = torch.stack([torch.tensor(np.pad(word[0], ((0, max_len \
#                     - word[0].shape[0]), (0, 0)), mode='constant', constant_values=0)) \
#                     for word in batch])
#         labels = torch.stack([torch.tensor(x[1]) for x in batch])
#         miss_chars = torch.stack([torch.tensor(x[2]) for x in batch])
#         lens = torch.tensor([x[0].shape[0] for x in batch], dtype=torch.long)
