...
def init_lstm(input_size: int, hidden_state_size: int) -> nn.Module:
    """
    Initialize an LSTM module.

    Args:
        input_size (int): Input size to the LSTM.
        hidden_state_size (int): Hidden state size for the LSTM.

    Returns:
        nn.Module
    """
    lstm = nn.LSTM(input_size, hidden_state_size)

    for name, param in lstm.named_parameters():
        if "bias" in name:
            nn.init.constant_(param, 0)  # Initialize biases to zero
        elif "weight" in name:
            nn.init.orthogonal_(param)  # Initialize weights orthogonally
...
def init_module(
    module: nn.Module, weight_init: Callable, \
        bias_init: Callable, gain: float = 1.0
) -> nn.Module:
    """
